...
<div class="p-1 pb-0 ml-0 pl-0">
    <h5 class="mt-2 text-primary"> Basic Info </h5>
</div>
</div>
...
<form @submit.prevent="formCheck" class="needs-validation">
            ...
            <div class="col-md-6">
                <div class="m-1 mt-3">
                    <label for="fname" class="pb-1 text-dark">Othername</label>
                <div class="input-group">
                    <span class="input-group-text"><i class="bi-person"></i></span>
                    <input type="text" v-model="othername"  maxlength="50" class="form-control" placeholder="Othername">
                </div>
                <small class="form-text text-muted"></small>
...
export default {
   ...
   preview: function(){
    ...
    update: function(){
        ...
        axios.post('/posts/user/updatename/', form)
        .then(response => {
        if(response.data.status==response.data.statusmsg){
        this.classname=response.data.classname
        this.alert=response.data.msg
