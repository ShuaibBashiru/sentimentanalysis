...
<div class="p-1 pb-0 ml-0 pl-0">
    <h5 class="mt-2 text-primary"> Web menu  (<span class="text-secondary">{{counter}}</span>)</h5>
</div>
</div>
...
<div class="btn-group m-2" title="" role="group" aria-label="First group"><a href="/secure/webnewmenu" class="btn btn-outline-primary text-center">  <i class="bi-plus" style="font-size: 1rem;"></i> New </a></div>
<div class="btn-group m-2" title="Menu" role="group" aria-label="First group">
    <button class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">Menu</button>
        ...
        <div class="col-md-12">
            ...
            <div class="table-responsive">
                ...
                <table class="table table-hover table-bordered" id="myTable">
                    ...
                    <td class="text-center" v-if="d['visibility']==1"> <i class="bi-check-square btn p-0 text-primary"></i></td>
                    <td class="text-center" v-else> <i class="bi-x-square btn p-0 text-danger"></i></td>
                    <td class="text-center"> <a :href="'/secure/updatewebmenu/'+d['menuName']+'/'+d['id']"><i class="bi-pencil-square btn p-0 text-primary"></i> </a></td>
                    <td class="text-center"> <a :href="'/secure/deletewebmenu/'+d['menuName']+'/'+d['id']"><i class="bi-trash btn p-0 text-danger"></i> </a></td>
                    </tr>
                      </tbody>
                </table>
            </div>
...
<form @submit.prevent="formCheckStatus" class="needs-validation">
      ...
      <div v-bind:class="'alert '+ classnamemodal +' p-2 ps-3 pe-3 m-0 mb-1 mt-1 text-center border-0'"> <span></span> {{alertmodal}} </div>
            ...
            <div class="col-md-6">
                ...
                <div class="m-1">
                <div class="input-group">
                   <select v-model="visibility" class="form-control" id="fitler" required>
                       <option disabled value="" selected>Select</option>
...
export default {
    ...
    updateStatus: function(){
        ...
        axios.post('/posts/adminwebmenu/update_status/', form)
        .then(response => {
   ...
   tokenize: function(){
    ...
    preview: function(){
        ...
        axios.get('/api/adminwebmenu/list/',{
              params:{
       ...
       filter: function(){
        ...
        this.isDisabled = true
        this.opacity = this.opacity_enable
       axios.get('/api/adminwebmenu/filter/', {
            params:{
       ...
       searchby: function(){
        ...
        this.isDisabled = true
        this.opacity = this.opacity_enable
       axios.get('/api/adminwebmenu/search/', {
            params:{
                'search': this.search
            }
        })
        ...
        axios.get('/api/adminwebmenu/download/', {
            params:{
                "filetype":"excel"
            }
