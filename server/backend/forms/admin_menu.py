...
from .admin_menu_model import AddNewMenu
...
def update(request):
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE admin_menus SET category=%s, menuName=%s, menu_description=%s, status_id=%s, "
                    "modified_by=%s, date_modified=%s, time_modified=%s "
                    "WHERE id=%s ",
                    [category, menuName, description, status_id, modified_by, date_modified, time_modified, keyid])
                transaction.commit()
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for keyid in lists:
                    total += 1
                    cursor.execute("UPDATE admin_menus SET status_id=%s, visibility=%s, "
...
def delete(request):
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                counter = cursor.execute("SELECT id FROM admin_menus WHERE id =%s AND menuName=%s", [keyid, itemName])
                if counter > 0:
                    cursor.execute("DELETE FROM admin_menus WHERE id=%s AND menuName=%s", [keyid, itemName])
                    transaction.commit()
