...
from forms.itemcategorymodel import AddNewItemCategory
    ...
    if request.method != 'POST':
        ...
        try:
            with connection.cursor() as cursor:
                counter = cursor.execute("SELECT  category_name FROM items_category WHERE category_name =%s", [keyid, ])

                if counter > 0:
                    exist += 1
                else:
                    gettime = datetime.datetime.now()
                    save_record = AddNewItemCategory()
...
def update(request):
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE items_category SET category=%s, category_name=%s, description=%s, status_id=%s, "
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for keyid in lists:
                    total += 1
                    cursor.execute("UPDATE items_category SET status_id=%s, visibility=%s, "
...
def delete(request):
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                counter = cursor.execute("SELECT id FROM items_category WHERE id =%s AND category_name=%s", [keyid, itemName])
                if counter > 0:
