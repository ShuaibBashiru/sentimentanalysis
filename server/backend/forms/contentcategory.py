...
from forms.contentcategorymodel import AddNewCategory
from authentication.writer import write_error

current_file = 'Category_forms'
...
def new_request(request):
    ...
    failed = 0
    exist = 0
    keyid = request.POST['category_name']
    if request.method != 'POST':
        ...
        try:
            with connection.cursor() as cursor:
                counter = cursor.execute("SELECT  category_name FROM content_category WHERE category_name =%s", [keyid, ])

                if counter > 0:
                    exist += 1
                else:
                    gettime = datetime.datetime.now()
                    save_record = AddNewCategory()
                    save_record.category_name = request.POST['category_name'].capitalize()
                    save_record.description = request.POST['description']
...
def update(request):
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            category_name = request.POST['category_name'].capitalize()
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE content_category SET category=%s, category_name=%s, description=%s, status_id=%s, "
                    "modified_by=%s, date_modified=%s, time_modified=%s "
                    "WHERE id=%s ",
                    [category, category_name, description, status_id, modified_by, date_modified, time_modified, keyid])
                transaction.commit()
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for keyid in lists:
                    total += 1
                    cursor.execute("UPDATE content_category SET status_id=%s, visibility=%s, "
...
def delete(request):
    ...
    if request.method != 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                counter = cursor.execute("SELECT id FROM content_category WHERE id =%s AND category_name=%s", [keyid, itemName])
                if counter > 0:
                    cursor.execute("DELETE FROM content_category WHERE id=%s AND menuName=%s", [keyid, itemName])
                    transaction.commit()
