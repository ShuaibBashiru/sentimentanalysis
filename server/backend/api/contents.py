...
def list_record(request):
    try:
        ...
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT t1.*, t2.category_name FROM feedback as t1 INNER JOIN items_category "
...
def list_search(request):
    try:
        ...
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT t1.*, t2.category_name FROM feedback as t1 INNER JOIN items_category "
                                     "as t2 ON t2.id=t1.category_id WHERE t1.comments like %s OR "
                                     "t2.category_name like %s ", ['%{}%'.format(search), '%{}%'.format(search)])
...
def preview(request):
    try:
        ...
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT t1.*, t2.category_name, t2.id as category_id FROM feedback as t1 INNER "
